...
if selected_tab == 'Загрузка':
    ...
    for date in pd.date_range(start=min(period_data['arrDate']), end=max(period_data['arrDate'])):
        bookings_on_date = period_data[(period_data['arrDate'] <= date) & (date < period_data['depDate'])]
    ...
    def create_recommendations(user_location, user_score, user_stars):
        ...
        st.write(
            f"Топ 5 рекомендаций отелей для гостя в Греции -'{user_location}' с рейтингом отзывов {user_score} и {user_stars} звезды:")
